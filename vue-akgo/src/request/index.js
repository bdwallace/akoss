...
const install = function (Vue) {
    ...
    axios.interceptors.response.use(
        response => {
            ...
            let dataCode = resData.code
            let dataMsg = resData.msg
            let dataData = resData.data
            if (response.data.code === port_code.success) {
                return Promise.resolve(response)
            } else if (response.data.code === port_code.unlogin) {
                setUserInfo(null)
                router.replace({name: "login"})
            }
            ...
            return Promise.reject({code: dataCode, msg: dataMsg, data: dataData})
        },
        error => {
            //错误时
            if (error.response) {
